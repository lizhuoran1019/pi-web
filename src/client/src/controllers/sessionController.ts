...
export class SessionController {
   ...
   * then deliver the text as the new branch's opening prompt. This is the first
   * moment the rewrite touches the server — until the user submits it, the whole
   * rewrite lives in the inline editor on the message itself, so changing their
   * mind costs nothing and leaves no trace.
   *
   * Failures are thrown rather than written to `state.error`: the inline editor
   * that called this is still on screen holding the text, so it is the right
   * place to show what went wrong and to try again from. The leaf is read
   * immediately before the mutation so the server's optimistic-concurrency check
   * still guards this entry point.
   */
  async rewriteMessage(entryId: string, text: string): Promise<void> {
    const state = this.getState();
    const session = state.selectedSession;
   ...
   * that already owns the text the user is about to send; `reportErrors` is false
   * when that caller shows failures inline next to the text instead of in the
   * session error bar, where the same message would appear twice.
   */
  private async runTreeNavigation(
    session: SessionInfo,
    request: SessionTreeNavigateRequest,
    openTree: SessionTreeSnapshot | undefined,
    options: { handBackEditorText: boolean; reportErrors?: boolean },
    ...
    try {
      ...
      if (reportErrors && this.isCurrentSessionSelection(session.id, machineId, selectionSeq)) this.setState({ error: String(error) });
      throw error;
    }
    ...
    try {
      ...
      if (reportErrors && this.isSelectedSessionIdentity(session.id, machineId)) this.setState({ error: String(error) });
      throw error;
    }
    if (authoritativeRefreshFailure !== undefined) {
      if (reportErrors && this.isSelectedSessionIdentity(session.id, machineId)) this.setState({ error: String(authoritativeRefreshFailure.error) });
